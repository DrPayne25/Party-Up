    ...
    useEffect(() => {
        const findUser = async () => {
            if (isAuthenticated) {
                ...
                await axios.get('http://34.208.228.216:8000/party_up_api/profile/?email=' + user.email)
                    .then(res => {
                        data = res.data;
                        try {
                            if (data[0].email === user.email) {
                                ...
                                // change this to our deployed db later, make sure it is an .env variable
                                axios.put('http://34.208.228.216:8000/party_up_api/profile/' + data[0].id, { logged_in: true })
